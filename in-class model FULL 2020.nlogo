...
employees-own [
  ...
  team_id ;; to make team members recognize each others
...
to setup
  ...
  ifelse Design_teams [
    ...
    if management [
      ask n-of (0.1 * initial_num_agents) employees [
        set color red
        set manager? 1
        set size 1
      ]
  ...
  ifelse design_teams [
    ask employees with [ manager? = 0 ] [
      ...
      if team_attitude > (1 + 0.5) [
        ...
        create-links-with other employees with [ team_id = ID
        and team_attitude > (1 + 0.5)] [
          set color yellow ]
      ]
    ]
    ...
    ask employees [
      ...
      if any? my-links [
        layout-spring employees links (#1 / 3) #2 (#3 * 3) ;; 0.1 0.1 0.3
      ]
    ]
  ]
...
to solve-problems
  ask employees [
    ...
    if any? my-links with [ color = sky ] [
      ...
      ifelse diff < [competence] of self [
        ...
        set shape "circle 2"
        ;; possible increase in competence?
      ] [
  ...
  ask problems [
    ...
    ;; possible decrease in competence?
  ]
...
to diffusion
  ask employees [
    if count link-neighbors with [ breed = employees
      and color = orange ] > (0.5 * count link-neighbors) [
      set shape "circle 2"
      set color orange
    ]
    if count link-neighbors with [ color = yellow
      and shape = "circle 2" ] > (0.5 * count link-neighbors) [
      set shape "circle 2"
    ]
  ]
...
"" ""
PENS
"sw-use" 1.0 0 -955883 true "" "plot count employees with [ color = orange ]"
"sw-no_use" 1.0 0 -13840069 true "" "plot count employees with [ color = yellow and shape = \"circle 2\" ]"
"regulars" 1.0 0 -1184463 true "" "plot count employees with [ shape = \"circle\" ]"
